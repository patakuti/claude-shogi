...
def analyze_position() -> dict:
    ...
    盤端または最初の駒(遮蔽物、自分の駒でも相手の駒でもそこで利きは止まる)までの
    全マスを収集し、遮蔽物の升目自体は含む。候補手を自分で挙げる前に、盤面全体で
    どこが大駒の利き筋に入っているかを俯瞰するための事前の危険地図であり、
    候補手ごとの安全性はdestination/own_attacked_afterで別途確認が必要
    (ピン・取り合いの最終損得は判定しない)。king_safetyと同じ理由で王手中でも
    通常どおり計算される。
    """
...
def verify_moves(
    ...
    opponent_fork_threats_after(§25.1): この手をpushした直後(応手を読む前、
    major_piece_drop_threats_afterと同じ時点)の局面に対する
    major_piece_fork_opportunities(相手視点、王手を伴う両取りも対象に含める)。
    非空なら、この手を指した直後に相手の飛・角が王手を伴う両取り(玉への当たり
    =王手そのものを1駒分、玉以外の当たりが1つ以上あれば両取りとして採用)を
    成立させられることを示す。材料点変化が良くても原則避けるべき候補。
    既知の限界は§24.1と同じ(動かした駒自身の利き以外による当たりは対象外、
    紐が1つでもあれば対象から外れる)。is_mateの候補には付けない。
    """
    board = _board_snapshot()
    if board is None:
        return {"ok": False, "error": "no_active_game"}
